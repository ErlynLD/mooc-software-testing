...
public class ChocolateBagsTest {
    ...
    @ParameterizedTest(name = "small={0}, big={1}, total={2}, result={3}")
    @CsvSource({"4, 3, 18, 3", "2, 1, 7, 2", "5, 4, 26, -1"})
    public void bigAndSmallBars2(int s, int b, int t, int e){
        ChocolateBags choco = new ChocolateBags();
        int resultado = choco.calculate(s, b, t);

        Assertions.assertEquals(e, resultado);
    }
    ...
    @ParameterizedTest
    @CsvSource({"1,3,7, -1", "3, 4, 12, 2"})
    public void justTrying(int s, int b, int t, int e){
        int result = new ChocolateBags().calculate(s,b,t);
